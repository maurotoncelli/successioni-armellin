...
- [~] Redigere informativa privacy, cookie policy e condizioni di vendita/T&C. Prime BOZZE IT pronte (DA VALIDARE da legale): `bozze_legali/Privacy_Policy_IT_BOZZA.md`, `bozze_legali/Condizioni_di_Vendita_TC_IT_BOZZA.md`, `bozze_legali/Recesso_e_Checkout_IT_BOZZA.md`. (Cookie policy definitiva generata dalla CMP iubenda.) **STATO**: i testi PRELIMINARI sono ora pubblicati e navigabili sul sito in modo data-driven (`web/src/content/legal.ts` -> pagine `/privacy`, `/termini-condizioni`, `/cookie-policy`, `/garanzia`; renderer `components/site/legal-doc.tsx`). Ogni pagina riporta l'avviso "testo preliminare in validazione" e la clausola "versione italiana fa fede". **DA FARE prima del go-live**: far validare i testi dal legale/DPO e rimuovere gli avvisi; definire le condizioni della Garanzia (casi/importi/tempi) e la retention dei lead non convertiti.
- [ ] **Confermare i dati identificativi obbligatori del footer** (oggi placeholder "DA CONFERMARE", gestiti data-driven in `footer.studio` + `settings`): **P.IVA, Codice Fiscale, numero REA, numero e Collegio di iscrizione all'Albo, PEC, email pubblica, telefono, forma giuridica esatta**. Sono mostrati nel footer e interpolati automaticamente nei testi legali (privacy/condizioni/cookie). Domanda aperta a Lorenzo: vedi `DOMANDE_PER_LORENZO.md`. Valutare anche un'email dedicata (es. `privacy@dominio`) per le richieste GDPR.
