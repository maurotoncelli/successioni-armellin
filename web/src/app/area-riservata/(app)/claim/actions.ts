...
export async function claimPracticeByCode(
  ...
  if (!isAuthConfigured || !isAdminConfigured) {
    return { ok: false, error: "Servizio non disponibile al momento." };
  }

  const code = normalizeCode(codeRaw);
  const email = emailRaw.trim().toLowerCase();
  if (!code || !email) {
    return { ok: false, error: "Inserisci codice pratica e email." };
  }
  ...
  if (!user) {
    return { ok: false, error: "Devi essere autenticato per collegare la pratica." };
  }
  ...
  if (authEmail !== email) {
    return {
      ok: false,
      error: `Sei connesso come ${authEmail}. Per questa pratica devi accedere con ${email}.`,
      needsEmail: email,
    };
  }
  ...
  if (error || !practice) {
    return {
      ok: false,
      error: "Nessuna pratica trovata con questo codice. Controlla e riprova.",
    };
  }
  ...
  if (!contactId) {
    ...
    if (!contactId) {
      return { ok: false, error: "Non siamo riusciti a collegare il contatto. Contatta lo studio." };
    }
    await admin
      .from("practices")
