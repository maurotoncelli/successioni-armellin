...
for (let i = 0; i <= 8; i++) {
  Object.assign(
    map,
    JSON.parse(fs.readFileSync(path.join(__dirname, `ru_map_${i}.json`), "utf8")),
  );
}
...
for (let i = 0; i <= 3; i++) {
  ...
  trOrdered.push(
    ...JSON.parse(
      fs.readFileSync(path.join(__dirname, `overlay_ru_${i}.json`), "utf8"),
    ),
  );
}
...
const articlesRu = walk(articlesIt);
...
const REVIEWED = ${emitTsString(articlesRu[0]?.reviewedBy ?? "Налоговые аспекты проверены присяжными налоговыми консультантами")};

const FONTE_ADE_SCHEDA: ArticleSource = {
  label: ${emitTsString(
    articlesRu.find((a) => a.sources?.[0])?.sources?.[0]?.label ??
...
const FONTE_ADE_IMPOSTE: ArticleSource = {
  label: ${emitTsString("Agenzia delle Entrate - Как уплачиваются налоги")},
...
const FONTE_NORMATTIVA: ArticleSource = {
  label: ${emitTsString("Normattiva - TUS Законодательный декрет 346/1990")},
...
fs.writeFileSync(path.join(root, "src/content/articles.ru.ts"), articlesTs);
...
fs.writeFileSync(path.join(root, "src/content/legal.ru.ts"), legalTs);

console.log("wrote articles.ru.ts and legal.ru.ts");
