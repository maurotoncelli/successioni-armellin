...
for (let i = 0; i <= 8; i++) {
  Object.assign(
    map,
    JSON.parse(fs.readFileSync(path.join(__dirname, `es_map_${i}.json`), "utf8")),
  );
}
...
for (let i = 0; i <= 3; i++) {
  ...
  trOrdered.push(
    ...JSON.parse(
      fs.readFileSync(path.join(__dirname, `overlay_es_${i}.json`), "utf8"),
    ),
  );
}
...
const articlesEs = walk(articlesIt);
...
const REVIEWED = ${emitTsString(articlesEs[0]?.reviewedBy ?? "Los aspectos fiscales han sido revisados por asesores fiscales colegiados")};

const FONTE_ADE_SCHEDA: ArticleSource = {
  label: ${emitTsString(
    articlesEs.find((a) => a.sources?.[0])?.sources?.[0]?.label ??
...
const FONTE_ADE_IMPOSTE: ArticleSource = {
  label: ${emitTsString("Agenzia delle Entrate - Cómo se pagan los impuestos")},
...
const FONTE_NORMATTIVA: ArticleSource = {
  label: ${emitTsString("Normattiva - TUS Decreto Legislativo 346/1990")},
...
fs.writeFileSync(path.join(root, "src/content/articles.es.ts"), articlesTs);
...
fs.writeFileSync(path.join(root, "src/content/legal.es.ts"), legalTs);

console.log("wrote articles.es.ts and legal.es.ts");
