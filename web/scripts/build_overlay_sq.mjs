...
for (let i = 0; i <= 8; i++) {
  Object.assign(
    map,
    JSON.parse(fs.readFileSync(path.join(__dirname, `sq_map_${i}.json`), "utf8")),
  );
}
...
for (let i = 0; i <= 3; i++) {
  ...
  sqOrdered.push(
    ...JSON.parse(
      fs.readFileSync(path.join(__dirname, `overlay_sq_${i}.json`), "utf8"),
    ),
  );
}
if (itOrdered.length !== sqOrdered.length) {
  console.error("length mismatch", itOrdered.length, sqOrdered.length);
  process.exit(1);
}
for (let i = 0; i < itOrdered.length; i++) map[itOrdered[i]] = sqOrdered[i];
...
const articlesSq = walk(articlesIt);
...
const REVIEWED = ${emitTsString(articlesSq[0]?.reviewedBy ?? "Aspektet fiskale të rishikuara nga kontabilistë")};

const FONTE_ADE_SCHEDA: ArticleSource = {
  label: ${emitTsString(
    articlesSq.find((a) => a.sources?.[0])?.sources?.[0]?.label ??
...
const FONTE_ADE_IMPOSTE: ArticleSource = {
  label: ${emitTsString("Agenzia delle Entrate - Si të paguani taksat")},
...
const FONTE_NORMATTIVA: ArticleSource = {
  label: ${emitTsString("Normattiva - TUS dekret ligjor 346/1990")},
...
fs.writeFileSync(path.join(root, "src/content/articles.sq.ts"), articlesTs);

const legalIt = JSON.parse(
  fs.readFileSync(path.join(__dirname, "legal_it.json"), "utf8"),
);
const legalSq = walk(legalIt);
findIt(legalSq, "legal");
...
for (const slug of Object.keys(legalSq)) {
  const doc = legalSq[slug];
...
fs.writeFileSync(path.join(root, "src/content/legal.sq.ts"), legalTs);

console.log("wrote articles.sq.ts and legal.sq.ts");
