#!/usr/bin/env node
/**
 * Build content_entries.zh.json from IT + string maps (agent-authored, no API).
 */
import fs from "fs";
...
const map = {};
for (let i = 0; i <= 8; i++) {
  const p = path.join(__dirname, `zh_map_${i}.json`);
...
const out = {
  _meta: {
    purpose:
      "Chinese (Simplified) courtesy translation of public site + area UI. Same keys as IT; IT remains legally authoritative (@07/@10).",
    locale: "zh",
    schema:
...
const outPath = path.join(root, "src/content/content_entries.zh.json");
fs.writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n");

const itK = new Set(it.entries.map((e) => e.collection + "." + e.key));
const zhK = new Set(out.entries.map((e) => e.collection + "." + e.key));
const miss = [...itK].filter((k) => !zhK.has(k));
