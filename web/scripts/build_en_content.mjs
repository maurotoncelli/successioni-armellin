#!/usr/bin/env node
/**
 * Build content_entries.en.json from IT + string maps (agent-authored, no API).
 */
import fs from "fs";
...
const map = {};
for (let i = 0; i <= 8; i++) {
  const p = path.join(__dirname, `en_map_${i}.json`);
...
const en = {
  _meta: {
    purpose:
      "English courtesy translation of public site + area UI. Same keys as IT; IT remains legally authoritative (@07/@10).",
    locale: "en",
    schema:
...
const outPath = path.join(root, "src/content/content_entries.en.json");
fs.writeFileSync(outPath, JSON.stringify(en, null, 2) + "\n");

const itK = new Set(it.entries.map((e) => e.collection + "." + e.key));
const enK = new Set(en.entries.map((e) => e.collection + "." + e.key));
const miss = [...itK].filter((k) => !enK.has(k));
console.log("wrote", outPath);
console.log("entries", en.entries.length, "missing keys", miss.length);
