...
for (let i = 0; i <= 8; i++) {
  Object.assign(
    map,
    JSON.parse(fs.readFileSync(path.join(__dirname, `tr_map_${i}.json`), "utf8")),
  );
}
...
for (let i = 0; i <= 3; i++) {
  ...
  trOrdered.push(
    ...JSON.parse(
      fs.readFileSync(path.join(__dirname, `overlay_tr_${i}.json`), "utf8"),
    ),
  );
}
...
const articlesTr = walk(articlesIt);
...
const REVIEWED = ${emitTsString(articlesTr[0]?.reviewedBy ?? "Vergi yönleri yeminli mali müşavirlerce gözden geçirilmiştir")};

const FONTE_ADE_SCHEDA: ArticleSource = {
  label: ${emitTsString(
    articlesTr.find((a) => a.sources?.[0])?.sources?.[0]?.label ??
...
const FONTE_ADE_IMPOSTE: ArticleSource = {
  label: ${emitTsString("Agenzia delle Entrate - Vergiler nasıl ödenir")},
...
const FONTE_NORMATTIVA: ArticleSource = {
  label: ${emitTsString("Normattiva - TUS Kanun Hükmünde Kararname 346/1990")},
...
fs.writeFileSync(path.join(root, "src/content/articles.tr.ts"), articlesTs);
...
fs.writeFileSync(path.join(root, "src/content/legal.tr.ts"), legalTs);

console.log("wrote articles.tr.ts and legal.tr.ts");
