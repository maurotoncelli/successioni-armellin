#!/usr/bin/env node
/**
 * Build content_entries.ru.json from IT + string maps (agent-authored, no API).
 */
import fs from "fs";
...
const map = {};
for (let i = 0; i <= 8; i++) {
  const p = path.join(__dirname, `ru_map_${i}.json`);
...
const out = {
  _meta: {
    purpose:
      "Russian courtesy translation of public site + area UI. Same keys as IT; IT remains legally authoritative (@07/@10).",
    locale: "ru",
    schema:
...
const outPath = path.join(root, "src/content/content_entries.ru.json");
fs.writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n");

const itK = new Set(it.entries.map((e) => e.collection + "." + e.key));
const ruK = new Set(out.entries.map((e) => e.collection + "." + e.key));
const miss = [...itK].filter((k) => !ruK.has(k));
