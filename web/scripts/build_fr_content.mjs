#!/usr/bin/env node
/**
 * Build content_entries.fr.json from IT + string maps (agent-authored, no API).
 */
import fs from "fs";
...
const map = {};
for (let i = 0; i <= 8; i++) {
  const p = path.join(__dirname, `fr_map_${i}.json`);
...
const out = {
  _meta: {
    purpose:
      "French courtesy translation of public site + area UI. Same keys as IT; IT remains legally authoritative (@07/@10).",
    locale: "fr",
    schema:
...
const outPath = path.join(root, "src/content/content_entries.fr.json");
fs.writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n");

const itK = new Set(it.entries.map((e) => e.collection + "." + e.key));
const frK = new Set(out.entries.map((e) => e.collection + "." + e.key));
const miss = [...itK].filter((k) => !frK.has(k));
