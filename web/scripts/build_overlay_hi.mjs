...
for (let i = 0; i <= 8; i++) {
  Object.assign(
    map,
    JSON.parse(fs.readFileSync(path.join(__dirname, `hi_map_${i}.json`), "utf8")),
  );
}
...
for (let i = 0; i <= 3; i++) {
  ...
  trOrdered.push(
    ...JSON.parse(
      fs.readFileSync(path.join(__dirname, `overlay_hi_${i}.json`), "utf8"),
    ),
  );
}
...
const articlesHi = walk(articlesIt);
...
const REVIEWED = ${emitTsString(articlesHi[0]?.reviewedBy ?? "कर संबंधी पहलुओं की समीक्षा शपथबद्ध कर सलाहकारों द्वारा की गई है")};

const FONTE_ADE_SCHEDA: ArticleSource = {
  label: ${emitTsString(
    articlesHi.find((a) => a.sources?.[0])?.sources?.[0]?.label ??
...
const FONTE_ADE_IMPOSTE: ArticleSource = {
  label: ${emitTsString("Agenzia delle Entrate - कर कैसे चुकाए जाते हैं")},
...
const FONTE_NORMATTIVA: ArticleSource = {
  label: ${emitTsString("Normattiva - TUS विधायी डिक्री 346/1990")},
...
fs.writeFileSync(path.join(root, "src/content/articles.hi.ts"), articlesTs);
...
fs.writeFileSync(path.join(root, "src/content/legal.hi.ts"), legalTs);

console.log("wrote articles.hi.ts and legal.hi.ts");
