#!/usr/bin/env node
/**
 * Build content_entries.sq.json from IT + string maps (agent-authored, no API).
 */
import fs from "fs";
...
const map = {};
for (let i = 0; i <= 8; i++) {
  const p = path.join(__dirname, `sq_map_${i}.json`);
...
const out = {
  _meta: {
    purpose:
      "Albanian courtesy translation of public site + area UI. Same keys as IT; IT remains legally authoritative (@07/@10).",
    locale: "sq",
    schema:
...
const outPath = path.join(root, "src/content/content_entries.sq.json");
fs.writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n");

const itK = new Set(it.entries.map((e) => e.collection + "." + e.key));
const sqK = new Set(out.entries.map((e) => e.collection + "." + e.key));
const miss = [...itK].filter((k) => !sqK.has(k));
