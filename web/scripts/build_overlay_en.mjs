...
const map = {};
for (let i = 0; i <= 8; i++) {
  Object.assign(map, JSON.parse(fs.readFileSync(path.join(__dirname, `en_map_${i}.json`), "utf8")));
}

const itOrdered = [];
const enOrdered = [];
for (let i = 0; i <= 3; i++) {
  itOrdered.push(...JSON.parse(fs.readFileSync(path.join(__dirname, `overlay_need_${i}.json`), "utf8")));
  enOrdered.push(...JSON.parse(fs.readFileSync(path.join(__dirname, `overlay_en_${i}.json`), "utf8")));
}
if (itOrdered.length !== enOrdered.length) {
  console.error("length mismatch", itOrdered.length, enOrdered.length);
  process.exit(1);
}
for (let i = 0; i < itOrdered.length; i++) map[itOrdered[i]] = enOrdered[i];

// identity for slugs / numeric tokens
for (const s of JSON.parse(fs.readFileSync(path.join(__dirname, "overlay_keep.json"), "utf8"))) {
  if (!(s in map)) map[s] = s;
}
...
function emitBlock(block, indent) {
  ...
  if (block.type === "ul" || block.type === "ol") {
    const items = block.items.map((it) => `${pad}    ${emitTsString(it)}`).join(",\n");
...
const articlesEn = walk(articlesIt);

// Check leftover Italian heuristics
const leftovers = [];
function findIt(v, pathHint) {
  if (typeof v === "string") {
    if (/[àèéìòù]/.test(v) || /\b(che|per|della|sono|non)\b/i.test(v)) {
      if (!(v in map) && !/^https?:/.test(v) && !/^\d/.test(v)) leftovers.push(pathHint + ": " + v.slice(0, 80));
    }
    return;
  }
  if (Array.isArray(v)) return v.forEach((x, i) => findIt(x, pathHint + `[${i}]`));
  if (v && typeof v === "object")
    Object.entries(v).forEach(([k, val]) => findIt(val, pathHint + "." + k));
}
findIt(articlesEn, "articles");
...
const REVIEWED = ${emitTsString(articlesEn[0]?.reviewedBy ?? "Tax aspects reviewed by chartered accountants")};

const FONTE_ADE_SCHEDA: ArticleSource = {
  label: ${emitTsString(
    articlesEn.find((a) => a.sources?.[0])?.sources?.[0]?.label ??
...
const FONTE_ADE_IMPOSTE: ArticleSource = {
  label: ${emitTsString("Agenzia delle Entrate - How to pay taxes")},
...
const FONTE_NORMATTIVA: ArticleSource = {
  label: ${emitTsString("Normattiva - TUS Legislative Decree 346/1990")},
...
for (const a of articlesEn) {
  ...
  const sources = a.sources
    .map((s) => {
      if (s.href.includes("dichiarazione-di-successione/imposte")) return "FONTE_ADE_IMPOSTE";
...
fs.writeFileSync(path.join(root, "src/content/articles.en.ts"), articlesTs);

const legalIt = JSON.parse(
  fs.readFileSync(path.join(__dirname, "legal_it.json"), "utf8"),
);
const legalEn = walk(legalIt);
findIt(legalEn, "legal");
...
for (const slug of Object.keys(legalEn)) {
  const doc = legalEn[slug];
...
fs.writeFileSync(path.join(root, "src/content/legal.en.ts"), legalTs);

console.log("wrote articles.en.ts and legal.en.ts");
