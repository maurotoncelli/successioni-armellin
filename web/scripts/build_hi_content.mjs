#!/usr/bin/env node
/**
 * Build content_entries.hi.json from IT + string maps (agent-authored, no API).
 */
import fs from "fs";
...
const map = {};
for (let i = 0; i <= 8; i++) {
  const p = path.join(__dirname, `hi_map_${i}.json`);
...
const out = {
  _meta: {
    purpose:
      "Hindi courtesy translation of public site + area UI. Same keys as IT; IT remains legally authoritative (@07/@10).",
    locale: "hi",
    schema:
...
const outPath = path.join(root, "src/content/content_entries.hi.json");
fs.writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n");

const itK = new Set(it.entries.map((e) => e.collection + "." + e.key));
const hiK = new Set(out.entries.map((e) => e.collection + "." + e.key));
const miss = [...itK].filter((k) => !hiK.has(k));
